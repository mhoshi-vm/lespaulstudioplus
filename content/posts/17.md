---
title: "k8sログ監視について色々検証してみた Part-2"
...
第一回 : [概要](../16.md)　　   
第二回 : [Fluentdで実装してみる]  **< いまここ**  
第三回 : [Wavefront Proxyで実装してみる](../19.md)  
第四回 : [考察](../20.md)

## はじめに

[概要](../16.md)でも記載したよう、「Kubernetesのログ監視」について、あまりに情報が少ないのでいったん検証して勉強してみるといったシリーズです。

この記事では、[Fluentd](https://www.fluentd.org/)のみを使い、とりあえず実装してみました。

## 注意点

今回紹介する記事は全てサポートされる方法を使っており、サポートとして問題ないはずですが、マニュアルなどに明記されていない手順のため、検証などは自己責任で実施いただくようお願いします。

また、この方法ですが、完璧ではなく問題もあるので、このページの"この方法での問題点"もご参照ください。
...
みての通り、`pod="agentupdater-workload-*"`のエントリーが大量にできているのですが、これ自体は、KubernetesのJobであり、pod側は定期的に実行されて消えていきます。
