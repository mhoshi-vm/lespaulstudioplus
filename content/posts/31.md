---
title: "SpringのアプリケーションでOpenAIのストリーミングアウトプットに対応したアプリケーションを作成する"
...
SpringのアプリケーションでOpenAIのストリーミングを有効にした出力を書きたいと思います。
...
なお、本当は `spring-boot-starter-webflux` 不要なはずです。細かい検証が原因ができていなく、原因が不明なのですが、執筆時点では、この対応が必要だった点を追記しておきます。
